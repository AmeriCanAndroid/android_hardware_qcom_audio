...
#ifdef QCOM_HTC_AUDIO
    , mVoiceVolume(1), mHACSetting(false), mBluetoothIdTx(0), mBluetoothIdRx(0),
        ...
        uint32_t devices, int *format, uint32_t *channels, uint32_t *sampleRate, status_t *status)
{
    { // scope for the lock
		...
		// KSB - This used to just be mOutput without VOIP enabled...
		...
		// KSB - This block was disabled without QCOM_VOIP
    ...
    if ((mode == AudioSystem::MODE_IN_CALL) && !isStreamOn(VOICE_CALL)) {
#if 0
		// KSB - Unusual... This may be important to why we don't get audio by default
        msm_start_voice();
#endif
        ALOGV("Going to enable RX/TX device for voice stream");
           ...
           if(new_tx_device !=INVALID_DEVICE && (enableDevice(new_tx_device,1) == -1))
               return -1;
#ifdef LEGACY_QCOM_VOICE
			// KSB - Another unusual thing... For HTC, without VOIP, we don't do this...
